...
import { Button, IconButton, Modal, Switch, TextField } from '@mui/material';
...
const TodosTable: React.FC = () => {
    ...
    useEffect(() => {
			dispatch(fetchTodosAsync()).catch(error => {
        ...
        <>
            <div className="my-4 flex flex-col md:flex-row items-center justify-between">
                <div className='flex md:flex-row mb-4'>
                    <TextField label="Фильтр по ID пользователя" variant="outlined" onChange={(e) => dispatch(setUserIdFilter(Number(e.target.value) || null))} className='md:!mr-8 !m-4' />
                    <TextField label="Фильтр по задачам" variant="outlined" onChange={(e) => dispatch(setTodoFilter(e.target.value))} className='!m-4' />
                </div>
            ...
            <Modal open={isOpen} onClose={() => setIsOpen(false)} className='flex justify-center'>
                <div className="p-4 m-auto w-2/3 md:w-1/3 bg-white rounded-md shadow-md">
                    <PageTitle text='Добавление новой задачи' />
                    {!isAdded && (
                        <>
                            <TextField fullWidth label="Новая задача" variant="outlined" value={newTodo} onChange={(e) => setNewTodo(e.target.value)} />
                    ...
                    {isAdded && (
                        <>
                            <div className='flex flex-col justify-center items-center'>
                                ...
                                />
                                <LabelMessage text='Задача успешно добавлена!' />
                            </div>
                        </>
                    )

                    }
                    <Button className={`!mt-8 ${isAdded ? '!bg-gray-500' : '!bg-emerald-600'}`} fullWidth variant="contained" onClick={handleAddTodo}>{isAdded ? '' : 'Добавить'}</Button>
                </div>
            </Modal>
        </>
    );
