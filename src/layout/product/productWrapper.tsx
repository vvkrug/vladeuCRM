...
const ProductWrapper:React.FC = () => {
  ...
  return (
    <div className='container flex flex-col'>
        ...
        <ProductsPieChart series={smartPhoneConfig.series} label={smartPhoneConfig.label} text='Смартфоны'/>
        </div>
        <div className='overflow-hidden' id='chart'>
        <ProductsBarChart series={automotiveConfig.series} categories={automotiveConfig.categories} text='Авто-аксессуары'/>
        </div>
        </div>
        <div className=''>
            <ProductsLineChart series={laptopDataConfig.series} categories={laptopDataConfig.categories} stockValue={laptopDataConfig.stockValue} text='Ноутбуки'/>
        </div>
    </div>
  )
}
